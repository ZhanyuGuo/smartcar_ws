...
with open('/home/gzy/pose_.txt', 'r') as file_handle:
    pos_1 = PoseStamped()
    pos_1.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_1.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_2 = PoseStamped()
    pos_2.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_2.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_3 = PoseStamped()
    pos_3.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_3.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_4 = PoseStamped()
    pos_4.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_4.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_5 = PoseStamped()
    pos_5.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_5.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_6 = PoseStamped()
    pos_6.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_6.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_7 = PoseStamped()
    pos_7.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_7.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_8 = PoseStamped()
    pos_8.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_8.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_9 = PoseStamped()
    pos_9.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_9.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_10 = PoseStamped()
    pos_10.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_10.pose.position.y = float(file_handle.readline().replace('\n', ''))

    pos_11 = PoseStamped()
    pos_11.pose.position.x = float(file_handle.readline().replace('\n', ''))
    pos_11.pose.position.y = float(file_handle.readline().replace('\n', ''))
...
def reach_sub_cb(data):
    ...
    if distance < 2.0 and reach_flag.data == False:
        # Info
...
def goal_sub_cb(data):
    ...
    # Remove the starting_point and ending_point
    way.pop(0)
